...
export default function ResumeTracker() {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/90">
      ...
      <label className="mt-4 block cursor-pointer rounded-lg border-2 border-dashed border-slate-300 bg-slate-50 p-4 text-center text-sm text-slate-600 transition hover:border-blue-400 hover:bg-blue-50 dark:border-slate-600 dark:text-slate-300">
      ...
      <div className="mt-5 space-y-2">
        {files.map((file) => (
          <article key={file._id} className="flex flex-wrap items-center justify-between gap-3 rounded border border-slate-200 bg-white p-3 text-sm shadow-sm dark:border-slate-700">
            <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => onDownload(file._id, file.fileName)} className="rounded border border-slate-300 bg-white px-2 py-1 text-xs text-slate-900 dark:border-slate-600 dark:text-slate-100">Download</button>
              <button type="button" onClick={() => onSetActive(file._id)} className="rounded border border-slate-300 bg-white px-2 py-1 text-xs text-slate-900 dark:border-slate-600 dark:text-slate-100">Set Active</button>
